function [Image] = sar3d_nfft(S, x, y, z, f, options)
...
kz = real(sqrt(4*k0.^2 - kx.^2 - ky.^2));

% Set evanescent modes to zero
WarpedSpectrum(kz == 0) = 0;

% for iz = 1:numel(z)
%     ImageSpectrum(:, :, iz, :) = abs(z(iz)) .* mean(WarpedSpectrum ...
%         .* exp(1j .* kz .* (abs(z(iz)))), 3);
% end

for xx = 1:numel(kx)
    xx
    for yy = find(max(kz(xx, :, :), [], 3))
        ImageSpectrum(xx, yy, :) = abs(z) .* nufft(...
            squeeze(WarpedSpectrum(xx, yy, :)), ...
            squeeze(kz(xx, yy, :)) ./ (-2*pi), ...
            abs(z)) ./ size(WarpedSpectrum, 3);
    end
end
