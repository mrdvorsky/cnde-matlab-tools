function [Image] = sar3dBi(S, x, y, z, f, options)
...
bistaticShift = exp(0.5j .* options.BistaticSeparationX .* kx) ...
    .* exp(0.5j .* options.BistaticSeparationY .* ky);

prevLayerMult = 1;
for ii = 1:numel(options.Thk)
    kz = real(sqrt(options.Er(ii)*k0.^2 - kx.^2 - ky.^2));

    % Set evanescent modes to zero
    WarpedSpectrum(4*options.Er(ii)*k0.^2 - kx.^2 - ky.^2 <= 0) = 0;

    for iz = find(layerIndices == ii)
        OneWaySpec = prevLayerMult .* exp(1j .* kz .* (abs(z(iz)) - zLayerStart(ii)));
        BiSpec = ifft2(fft2(OneWaySpec .* bistaticShift) ...
            .* fft2(OneWaySpec .* conj(bistaticShift)));

        BiSpec = exp(1j .* angle(BiSpec));

        ImageSpectrum(:, :, iz, :) = abs(z(iz)) .* mean(BiSpec .* WarpedSpectrum, 3);
    end
