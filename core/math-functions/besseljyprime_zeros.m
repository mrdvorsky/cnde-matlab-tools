...
% same, so that besseljy returns a "shifted besseljprime". More
% specifically, the asymtotic behavior at infinity will be 
% "sqrt(2/(pi*x)) cos(x - c)", where "c" is the required phase shift. This
% matches the behavior of besseljprime and besselyprime.
%
...
for ii = 1:numel(k)
    ...
    % Choose sign.
    val_sign = (-1).^(ii + 1 + (nu==0)) ...
        .* sign(besseljy(alpha(ii), beta(ii), nu, k(ii)*r2));
    alpha(ii) = val_sign * alpha(ii);
    beta(ii) = val_sign * beta(ii);
end
